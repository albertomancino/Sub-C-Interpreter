...
void SymbolTableCopy (struct SymbolTable * SymbolTab, struct SymbolTable * newSymbolTab){
  ...
  for (int i = 0; i < SymbolTab -> elements; i++){
    ...
    char * identifier = malloc(sizeof(char)*strlen(variable -> identifier)+1);
    strcpy(identifier, variable -> identifier);
    if (variable -> type == INT_)       SymbolTable_Add(newSymbolTab, identifier, INT_, 0, 0);
    else if (variable -> type == CHAR_) SymbolTable_Add(newSymbolTab, identifier, CHAR_, 0, 0);
...
void Change_ArrayDim (char * identifier, int new_dim){

  struct SymbolTable_Node * ST_Node = SymbolTable_RetrieveVar(identifier);
  ST_Node -> arrayDim = new_dim;
  if (ST_Node -> type == INT_V_) ST_Node -> varPtr.intPtr = (int*)malloc(sizeof(int)*new_dim);
  else if (ST_Node -> type == INT_V_) ST_Node -> varPtr.intPtr = (char*)malloc(sizeof(char)*new_dim);
}
////////////////////////////////////////////////////////////////////////////////
