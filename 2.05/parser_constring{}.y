    ...
    // array initializer to assign
      ...
      // array
        ...
        for (int i = 0; i < assignments; i++){
          ...
          // a string must be split in characters
          if (value -> node.Expr -> exprType != STR){
            // creating the assignement node
            ...
            printf("STRINGA\n" );
            struct TreeNode * string_value;
            char * string = value -> node.Expr -> exprVal.stringExpr;

            for (int j = 0; j < strlen(string); j++){
              if (i < assignments){

                string_value = create_ExprNode(C, 0, &string[j], NULL, NULL, 0);
                // creating the assignement node
                struct TreeNode * assignmentNode = create_AssignementNode(MainNode, variable, string_value);
                // linking assignement node
                TreeNodeList_Add(newTreeNode -> child_list, assignmentNode);
            ...
            // adding string end character
            if (i < assignments){
              char end_string = '\0';
              string_value = create_ExprNode(C, 0, &end_string, NULL, NULL, 0);
              // creating the assignement node
              struct TreeNode * assignmentNode = create_AssignementNode(MainNode, variable, string_value);
              // linking assignement node
              TreeNodeList_Add(newTreeNode -> child_list, assignmentNode);
